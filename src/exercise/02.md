...
Hook flow from example/hook-flow.png

React’s useState hook allows you to pass a function instead of the actual value,
and then it will only call that function to get the state value when the
component is rendered the first time. This is to avoid performance bottleneck
when initializing state using a value calculated by an expensive function.

Blog post:
https://kentcdodds.com/blog/use-state-lazy-initialization-and-function-updates?ck_subscriber_id=363845494

Example code sandbox for setting state using value vs using function:
https://codesandbox.io/s/demo-for-usestate-lazy-initialization-and-function-updates-forked-ksgo7?file=/src/App.js

Assign default value to {} when destructure object to avoid error Useful Blog
post: https://jacobparis.com/blog/destructure-arguments
